...
public class DescriptorCalculator  {
    ...
    public Dataset calculateMoleculeDescriptors(List<String> sdfFileName, String sourceName, String sourceURI) throws CDKException, IOException {
            ...
            while (iter.hasNext()) {
                ...
                if (result instanceof DoubleResult) {
                    if (!String.valueOf(((DoubleResult) result)).equals("NaN") && !String.valueOf(((DoubleResult) result)).equals("NA"))
                        calulationMap.put(value.getNames()[0], ((DoubleResult) result).doubleValue());
                }
                else if (result instanceof IntegerResult) {
                    if (!String.valueOf(((IntegerResult) result)).equals("NaN") && !String.valueOf(((IntegerResult) result)).equals("NA"))
                        calulationMap.put(value.getNames()[0], ((IntegerResult) result).intValue());
                }
                    ...
                    for (int i = 0; i < result.length(); i++)
                    {
                        if (!String.valueOf(((DoubleArrayResult) result).get(i)).equals("NaN") && !String.valueOf(((DoubleArrayResult) result).get(i)).equals("NA"))
                            calulationMap.put(value.getNames()[i],((DoubleArrayResult) result).get(i));
                    }
                    ...
                    for (int i = 0; i < result.length(); i++)
                    {
                        if (!String.valueOf(((IntegerArrayResult) result).get(i)).equals("NaN") && !String.valueOf(((IntegerArrayResult) result).get(i)).equals("NA"))
                            calulationMap.put(value.getNames()[i],((IntegerArrayResult) result).get(i));
                    }
            }
        ...
        for (IDescriptor descriptor : myDescriptors) {
            ...
            for (String name : descriptor.getDescriptorNames()) {
                ...
                featureInfo1.setConditions(conditions);
                featureInfo1.setName(name+" descriptor of feature with name "+sourceName);
                if (sourceURI!=null)
                    featureInfo1.setName(featureInfo1.getName().concat(" and URI "+sourceURI));
                featureInfo1.setCategory(Dataset.DescriptorCategory.CDK);
                featureInfo1.setURI(name);
                features.add(featureInfo1);
            }
